                ...
                if (wasFingerDownLastFrame == true && TouchWrapper.IsFingerDown == false)
                {
                    ...
                    if (wasDraggingLastFrame == false && wasPinchingLastFrame == false)
                    {
                        ...
                        for (int i = 0; i < TouchWrapper.Touches.Count; i++)
                        {
                            ...
                            {
                                if (EventSystem.current.IsPointerOverGameObject(TouchWrapper.Touches[i].FingerId)) return;
                                if (TouchWrapper.PointIsOverUI(TouchWrapper.Touches[i].Position.x, TouchWrapper.Touches[i].Position.y)) return;
                            }
                            if (TouchWrapper.PointIsOverUI(lastFinger0DownPos.x, lastFinger0DownPos.y)) return;
                        }
                        ...
                        {
                            gameObject.GetComponent<MobileTouchCamera>().isLongClick = isLongTap;

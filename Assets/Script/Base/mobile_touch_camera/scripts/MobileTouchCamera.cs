...
public class MobileTouchCamera : MonoBehaviourWrapped
{
    ...
    private float camOverzoomMargin = 0.5f;
    [SerializeField]
...
#pragma warning restore 649
    ...
    [SerializeField]
    public float mouseZoomFactor = 1;
    [SerializeField]
    ...
    private float keyboardRepeatDelay = 0f;
    #endregion
    ...
    private float customZoomSensitivity = 1f;
    [SerializeField]
    ...
    public bool zoomDoubleClick = false;
    ...
    {
        if (GameManager.gameState != GameState.PLAYING) return false;
    ...
    {
        if (GameManager.gameState != GameState.PLAYING) return newPosition;
    ...
    public void Reset()
    {
        ...
        {
            CamZoomMin = 1;
            CamZoomMax = maxOrthographicSize;
            CamOverzoomMargin = 1;
            Cam.orthographicSize = maxOrthographicSize;
            CamZoom = maxOrthographicSize;
        }
        else
        {
            CamZoomMin = 1;
        ...
        if (IsPinching == true)
        {
            ...
            if (isTiltModeEvaluated == true)
            {
                ...
                if (isPinchTiltMode == false || isPinchModeExclusive == false)
                {
                    ...
                    {
                        //Zoom
                        float zoomFactor = (pinchDistanceStart / Mathf.Max(((pinchDistanceCurrentLerp - pinchDistanceStart) * customZoomSensitivity) + pinchDistanceStart, 0.01f));
    ...
    private void UpdatePosition(float deltaTime)
    {
        ...
        //Debug.Log(IsDragging + "  " + IsPinching);
    ...
    {
        #endregion
        keyboardZoomFactor = ((Cam.orthographicSize / 10) > 5 ? 5 : (Cam.orthographicSize / 10)); ;
        mouseZoomFactor = ((Cam.orthographicSize / 10) > 5 ? 5 : (Cam.orthographicSize / 10));
        camFollowFactor = ((maxOrthographicSize / Cam.orthographicSize) < 2.5f ? 2.5f : (maxOrthographicSize / Cam.orthographicSize));
    }
    ...
    public void LateUpdate()
    {
        ...
        {
            if (TouchWrapper.Touches[i] != null)
                if (TouchWrapper.PointIsOverUI(TouchWrapper.Touches[i].Position.x, TouchWrapper.Touches[i].Position.y)) return;
        }
        //Pinch.
...
#pragma warning restore 162
    ...
    {
        //if (isDoubleClick &&  HiddenManager.gameState == HiddenManager.GameState.PLAYING && Cam.orthographicSize > 1 && zoomDoubleClick)
        //{
        //        StartCoroutine(DoubleClickZoom(Cam.orthographicSize));
        //        GameMaster.Instance.currentGame.ChangeState(HiddenManager.GameState.HINT);
        //}
        if (isLongTap == true)
        {
            return;
        }
        //if (OnPickItemDoubleClick != null && isDoubleClick && HiddenManager.gameState == HiddenManager.GameState.PLAYING && !zoomDoubleClick)
        //{
        //    zoomDoubleClick = true;
        //    setCamHint(clickPosition, Cam.orthographicSize - Cam.orthographicSize / 5, null);
        //}
    ...
    private IEnumerator ZoomToTargetValueCoroutine(float target)
    {
        ...
        if (Mathf.Approximately(target, CamZoom) == false)
        {
            ...
            const float duration = 0.000001f;
    ...
    public void setCamPosLimit(GameManager gameManager, float y)
    {
        boundaryMin.x = gameManager.mapRenderer.transform.position.x - GameManager.Instance.mapRenderer.bounds.size.x / 2f;
        boundaryMax.x = gameManager.mapRenderer.transform.position.x + GameManager.Instance.mapRenderer.bounds.size.x / 2f;

        boundaryMin.y = gameManager.mapRenderer.transform.position.y - GameManager.Instance.mapRenderer.bounds.size.y / 2f - y;
        boundaryMax.y = gameManager.mapRenderer.transform.position.y + GameManager.Instance.mapRenderer.bounds.size.y / 2f;
    }
    ...
    {
        StartCoroutine(OrthographicSize());
    }

    IEnumerator OrthographicSize()
    {
        Cam.transform.position = new Vector3(0, 0, -10);
        while (Cam.orthographicSize < maxOrthographicSize)
        {
            Cam.transform.position = new Vector3(0, 0, -10);
            if ((Cam.orthographicSize / 2 ) < 2)
            {
                Cam.orthographicSize += 0.5f;
            }
            {
                Cam.orthographicSize += ((Cam.orthographicSize / 2) > 5 ? 5 : (Cam.orthographicSize / 2));
            }
    ...
    public void setLoadMap(Vector3 pos, float orthographicSize)
    {
        Cam.orthographicSize = orthographicSize;
        transform.position = pos;
        ComputeCamBoundaries();
    }
    //IEnumerator OrthographicSizeHint(Vector3 pos, float orthographicSize, Item item)
    //{
    //    bool check = false;
    //    int percent = 17;
    //    float time = 0.009f;
    //    float dur = 1.5f;
    //    if (item != null) GameMaster.Instance.uIManager.Hint(item.transform.position, item);
    //    if (GameMaster.Instance.currentGame.map.id == 1 && GameMaster.Instance.currentGame.map.count == GameMaster.Instance.currentGame.map.listItem.Length - 1)
    //    {
    //        percent = 65;
    //        time = 0.012f;
    //        dur = 3f;
    //    }
    //    if (pos == new Vector3(0, 0, -10)) check = true;
    //    else
    //    {

    //        transform.DOMove(pos, dur).OnComplete(() =>
    //       {
    //           check = true;
    //           transform.DOKill();
    //       });
    ...
    //    {
    //        orthographicSize = Mathf.Min(orthographicSize, maxOrthographicSize);
    //        while (Cam.orthographicSize < orthographicSize || !check)
    //        {
    //            if (Cam.orthographicSize < orthographicSize)
    //            {
    //                if (Cam.orthographicSize + Cam.orthographicSize / percent < orthographicSize)
    //                {
    //                    Cam.orthographicSize += Cam.orthographicSize / percent;
    //                } else
    //                {
    //                    Cam.orthographicSize = orthographicSize;
    //                }
    ...
    //    ComputeCamBoundaries();
    //    //zoomDoubleClick = true;
    //    GameMaster.Instance.currentGame.ChangeState(HiddenManager.GameState.PLAYING);
    //}

    public void SetMaxOrthographicSize()
    {
        float ratio = ((float)Screen.width / Screen.height) / 0.5625f;
        maxOrthographicSize = 3600 / ratio;
    }

    IEnumerator DoubleClickZoom(float orthographicSize)
    {
        while (Cam.orthographicSize > orthographicSize - orthographicSize/1.2f && Cam.orthographicSize != 1)
        {
            float camSizeDiff = DoEditorCameraZoom(Cam.orthographicSize / 20);
            Vector3 intersectionScreenCenter = GetIntersectionPoint(Cam.ScreenPointToRay(new Vector3(Screen.width * 0.5f, Screen.height * 0.5f, 0)));
            Vector3 pinchFocusVector = GetIntersectionPoint(Cam.ScreenPointToRay(Input.mousePosition)) - intersectionScreenCenter;
            float multiplier = (1.0f / CamZoom * camSizeDiff);
            Transform.position += pinchFocusVector * multiplier;
    ...
    IEnumerator SetZoomDoubleClick(bool active, float time)
    {

        yield return new WaitForSeconds(time);
        zoomDoubleClick = active;
    }

    public void SetZoomDoubleClick()
    {
        zoomDoubleClick = false;
        StartCoroutine(SetZoomDoubleClick(true, 2f));
    }
}
